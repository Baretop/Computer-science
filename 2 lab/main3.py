...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        #abs = np.power(self.nodes[0, :] - 5, -2, dtype= float) + np.power(self.nodes[1, :] - 25, -2, dtype=float) + np.power(self.nodes[2, :] - 25, -2, dtype=float)
        self.smth = (100 - 5 * self.nodes[0] - self.nodes[1] - self.nodes[2])
    ...
    def move(self, tau):
        ...
        omega = 1
        self.velocity[1] = self.nodes[2] * omega
        self.velocity[2] = self.nodes[1] * (-1) * omega

        #for i in range(size(self.nodes[0])):
        #    abs = pow((self.nodes[0][i] - 5), 2
        #abs = np.power(self.nodes[0, :] - 5, -2, dtype = float) + np.power(self.nodes[1, :] - 25, -2, dtype = float) + np.power(self.nodes[2, :] - 25, -2, dtype = float)
        self.time += tau
        self.smth = math.cos(5 * self.time) * (100 - 5 * self.nodes[0] - self.nodes[1] - self.nodes[2])
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("shur-step-" + str(snap_number) + ".vtu")
        writer.Write()
...
try:
    ...
    gmsh.merge(os.path.join(path, 'shur.stl'))
except:
...
gmsh.model.mesh.field.setString(f, "F", "1.3")
